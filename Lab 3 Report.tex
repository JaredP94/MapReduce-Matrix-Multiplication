...
In order to perform matrix multiplication for large matrices, a MapReduce algorithm was implemented. The algorithm consists of two distinct parts, namely the \verb|map()| function and \verb|reduce()| function. 

The \texttt{map} function takes as an input a key and a value and emits key-value pairs. The \texttt{reduce} function then makes use of the output from the \texttt{map} function. It takes as inputs a key and a list of values associated with the key and performs an action of the values. It also emits key-value pairs.

MrJob is a MapReduce framework which provides an environment which allows multi-step MapReduce jobs to be written in Python and run on a variety platforms~\cite{mrjob}. A job is defined by class which inherits from \texttt{MRJob}, and which contains the functions which define the \texttt{steps} of the job. Each \texttt{step} consists of a \texttt{mapper}, a \texttt{combiner} and a \texttt{reducer}, of which you must use at least one~\cite{mrjob}.
...
In both the algorithms, the \texttt{map} function transforms the input file into a format which can be used by the reducer to perform the necessary calculations. The input data to the map function is a line from the text file in the form \textit{row column value}. The data is then assigned a key so that a key-value pair can be yielded from the function.

The \texttt{reduce} function performs computations on the key-value pairs produced by the \texttt{map} function in order to produce the product matrix.
...
In the reduce function in the first step, for each key j, the corresponding values ($m_{ij}$ and $n_{jk}$) are multiplied and the key-value pair j, $(i, k, m_{ij}n_{jk})$ is emitted. The output of the reduce function is then used as the input to the map function in the second step.

In the second step, a key-value pair of the form (i,k) is created for each element and assigned the combined value $m_{ij}n_{jk}$ in the mapper stage. Each i value will represent the row index of the product matrix and k representing the column index. 
